...
Zone_time = [1200, 1800, 1800] #seconds of time have per zone will run
...
Zone_Switch = [1, 1, 1] # Is this have able if needed? 0 = no, 1 = yes
...
''''
    ...
    if (time.monotonic() - Zone_On_Time[i]) >= Zone_time[i] :
        ...
        Zone_Off_Time[i] = time.monotonic()
        print("Heat Turned off for =", Zone_Name[i])
        print("Heat turned off for being greater than run time desired")
        print (time.ctime())'''
